...
md"""
`-` 두 지수분포에 대한 cdf(누적분포함수)는 아래와 같다. 
"""
...
# ╔═╡ a00d4727-6040-4d84-8f79-2b5a9a28e84e
md"""
아래로 떨어진 애들을 히스토그램으로 그려보면 지수분포를 띄지 않을까?
"""
...
let
	...
	p1 = plot(x -> -exp(-x)+1,0,20, title="평균=1") # 평균이 1인 지수분포의 cdf
	scatter!([0,0,0,0,0],u) # 이제 이 값들에서 출발하여 x에 떨어뜨릴 것임. 
	scatter!(Finv.(u),[0,0,0,0,0]) # 떨어진 x의 값들.
	p2 = plot(x -> -exp(-x/5)+1,0,20, title="평균=5") # 평균이 5인 지수분포의 cdf
	scatter!([0,0,0,0,0],u)
...
let
	...
	# 평균이 1인 지수분포의 cdf의 역함수
	# 아래로 떨어진 x값들을 의미함
	p2= histogram(rand(Exponential(1),10000))
	plot(p1,p2,layout=(1,2))  
end

# ╔═╡ 0e02bdaf-01a8-48dc-bdc9-baf76273b933
md"""
평균이 1인 지수분포와 유사하다.
"""
...
- 이해를 위해서 $t=1,s=9$ 대입 => $P(X>1)=P(X>10 | X>9)$
  - 아래 두 줄을 이해하기에 앞서 지수분포의 개념에 대해 다시 review
  - 지수분포 ? 특정한 이벤트의 발생 후, 그 다음에 같은 이벤트가 발생할 때까지 걸리는 시간에 대한 분포 
  - 즉, 확률변수 X가 1이라면 P(X>1)일 확률이 의미하는 건 시간 1을 기다렸음에도 이벤트가 발생하지 않았을 확률을 의미함
...
# ╔═╡ fec67843-a551-41d5-abc8-2f0a1ebf30a6
md"""
-----
"""
...
rand(Exponential(1),1000000) |> mean

# ╔═╡ d44bf5cd-7b8e-4e18-86fb-25e0e413e86e
md"""
##### 예제2: 아래를 계산하라. 
$\int_0^{\infty} x^2 e^{-x}dx=?$
- 제곱한 것들의 평균으로 해석할 수 있음
"""
...
# ╔═╡ 8b38a024-2559-4177-a9a1-bd2c7be3f731
let 
	N=5000
	X = rand(Normal(0,1),N)
	Y = rand(Normal(0,1),N)
	p1= (X.^2 + Y.^2)./2 |> histogram
	p2= histogram(rand(10000) .|> x -> -log(1-x))
	plot(p1,p2,layout=(1,2))
end 
